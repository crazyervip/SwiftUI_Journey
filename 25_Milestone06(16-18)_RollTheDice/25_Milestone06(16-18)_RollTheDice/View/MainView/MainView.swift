...
struct MainView<GenericRolls: Rolls>: View {
    ...
    var body: some View {
        ...
        let diceSidesWithCallback = Binding<Int>(
            get: { self.diceSides },
            set: {
                self.diceSides = $0
                self.resetPositions()
        ...
        let animationCompleteWithCallback = Binding<Bool>(
            ...
            set: {
                ...
                if self.animationComplete {
                    ...
                    self.rolls.insert(roll: Roll(diceSides: self.diceSides, result: result, total: total))
        ...
        return VStack {
            ...
            SidesPicker(diceSides: diceSidesWithCallback,
                        rollDisabled: rollDisabled)
                .padding()
            ...
            ZStack {
                ...
                VStack {
                    ...
                    RollButton(rollDisabled: rollDisabled,rollAction: rollAction)
                        .padding()
                }
            }
        }
    ...
    private func rollAction() {
        ...
        for i in 0..<self.diceNumber {
            ...
            self.selectedSides[i] = Int.random(in: 1...self.diceSides)
...
struct MainView_Previews: PreviewProvider {
    static let rolls = [
        Roll(diceSides: 20, result: [18, 15, 19, 17, 16, 19], total: 104),
        Roll(diceSides: 6, result: [1, 3, 4], total: 8),
        Roll(diceSides: 100, result: [95, 45, 21, 21, 32], total: 214),
        Roll(diceSides: 8, result: [4, 8, 3], total: 15),
        Roll(diceSides: 4, result: [1, 1, 3, 2, 4], total: 11),
        Roll(diceSides: 6, result: [4, 5, 2, 6, 3, 3], total: 23),
        Roll(diceSides: 6, result: [2, 1, 2, 6, 5, 4], total: 20),
        Roll(diceSides: 6, result: [5, 5, 5, 1, 4, 2], total: 22),
        Roll(diceSides: 6, result: [5, 3, 5, 4, 4], total: 21)
    ]
